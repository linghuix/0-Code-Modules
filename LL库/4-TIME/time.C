...
void TIME2_Configuration(void)
{
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);                    /*定时器TIM3时钟使能*/
    
    TIM_TimeBaseStructure.TIM_Period = 10000;                               /*设置在下一个更新事件装入活动的自动重装载寄存器周期的值,计数到10000为10ms*/
    
    TIM_TimeBaseStructure.TIM_Prescaler =(72-1);                            /*设置用来作为TIMx时钟频率除数的预分频值 72MHz/72 = 1000000Hz */
...
void TIM_interrupt(void)
{
    ...
    if(flag == 0xff) flag = 0;                                  //设定标志位

    printf("%d\r\n\r\n", TIM_GetITStatus(TIM2, TIM_IT_Update)); //验证中断服务时间是否小于设定时间
}
